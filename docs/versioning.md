...
How `luci-app-prism` version numbers are built, and the rules any change to
the packaging pipeline has to keep.

This is the single normative source. `CONTRIBUTING.md` summarises it,
`package.sh` and `snapshot.yml` carry only the invariants local to the lines
they sit on, and everything that can be stated as an assertion instead lives
in `.github/workflows/version-check.sh`, which both CI workflows run. Where
...
`r`, so a bare `0.8.3-1` ranks *below* an SDK- or feed-built `0.8.3-r1` of the
identical source.
...
1. **`PRISM_VERSION` is set** — `release.yml` sets it from the pushed tag,
...
2. **A `v*` tag is reachable from HEAD** — `<highest-tag>_git<TS>`, where
   ...
   `PKG_RELEASE`: snapshots following `v0.8.3-r2` are `0.8.3_git<TS>-r2`,
...
commit — which is exactly what a squashed history produces — so it can return
`v0.2.0` where `v0.8.3` exists and send the version backwards.
...
- Two branches the same number of commits past the tag produce an identical
  version. Every branch publishes to one shared rolling asset, so a tester
  moving between them can end up installing nothing.
...
resolves `PKG_RELEASE="${PRISM_RELEASE:-$PKG_RELEASE}"`, so anything CI sets
wins over the Makefile. Snapshot ordering belongs in the version body, which
is what the timestamp is for.

`PRISM_RELEASE` exists solely for `release.yml`'s `v<X.Y.Z>-r<N>` tags.
`snapshot.yml` deliberately sets nothing.
...
- **`-r<N>` is a terminal token** (`TOKEN_REVISION_NO`). Nothing may follow
  it, so `0.8.3-r2_git2-r1` does not parse and `apk mkpkg` rejects the
  package outright. Never build a version body out of a `v<X.Y.Z>-r<N>` tag
  without splitting the `-r<N>` off first.
- **`-` is not otherwise legal.** Debian's `0.9.0-1` is rejected.
...
| Comparison | apk | opkg |
|---|---|---|
| `0.9.0_git<TS>` vs `0.9.0` | `>` | `>` — agree |
| `0.9.0-r1` vs `0.9.0-r2` | `<` | `<` — agree |
...
`.github/workflows/version-check.sh` asserts the rules above against apk's
